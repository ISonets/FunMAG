#!/usr/bin/env python3
"""
Compute median coverage across samples for bin3C bins.
Usage: python median_cov_bin3c.py depth.tsv bin3c_fasta_dir output.tsv
"""
...
def main():
    if len(sys.argv) != 4:
        print("Usage: median_cov_bin3c.py <depth.tsv> <bins_dir> <output.tsv>", file=sys.stderr)
        sys.exit(1)
    ...
    # Remove variance columns and length/totalAvgDepth
    cols_to_drop = [c for c in depth.columns if c.endswith('var')]
    depth = depth.drop(columns=cols_to_drop)
    # Drop columns at index 1 and 2 (contigLen, totalAvgDepth)
    if depth.shape[1] > 3:
        depth = depth.drop(depth.columns[[1, 2]], axis=1)

    bin_files = [f for f in os.listdir(bins_dir) if f.endswith('.fna')]
    if not bin_files:
        with open(out_file, 'w') as f:
            f.write("bin\tmedian_coverage\n")
        return

    pattern = re.compile(r'NODE_\d+_length_\d+_cov_\d+\.\d+')
    results = {}
